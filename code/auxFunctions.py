...
def plot_desicion_threshold(
    ...
    fig.suptitle(f"Probability-based threshold with PRC: {dataset_name} {strain}")
    plt.tight_layout()  # Adjusts subplot params so that subplots fit into the figure area.
    plt.show()


def calc_and_plot_prob_threshold(
    data,
    classifier,
    precision,
    recall,
...
def analyze_all_datasets(datasets, metadata, bPlotOnly=False, bPlotThreshold=False):
    ...
    for dataset_name in metadata[dataset_col].unique():
        ...
        if len(strains) > 1:
            strains = list(
                set(strains) - set(["Influenza"])
            )  # denotes an MFC calculation in the original dataset
        # print(strains)
